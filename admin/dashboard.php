...
<body>
    ...
    if (isset($_POST['deletebatch'])) {
        $batch_id = $_POST['deletebatch'];

        // Get class_id and room_id associated with the selected batch
        $select_batch_info_query = "SELECT class_id, room_id FROM batch WHERE batch_id = '$batch_id'";
        $select_batch_info_result = mysqli_query($conn, $select_batch_info_query);

        if ($select_batch_info_result && mysqli_num_rows($select_batch_info_result) > 0) {
            $batch_info_row = mysqli_fetch_assoc($select_batch_info_result);
            $class_id = $batch_info_row['class_id'];
            $room_id = $batch_info_row['room_id'];

            // Delete all batch records associated with the selected class and room combination
            $delete_batch_query = "DELETE FROM batch WHERE class_id = '$class_id' AND room_id = '$room_id'";
    ...
    <div class="wrapper">
        <nav id="sidebar">
            ...
            <ul class="list-unstyled components">
                ...
                <li>
                    <a href="import_students.php"><img src="https://img.icons8.com/ios-filled/25/ffffff/import.png" />
                        Import Students</a>
                </li>
                ...
                <li>
                    <a href="dashboard.php" class="active_link"><img
                            src="https://img.icons8.com/nolan/30/ffffff/summary-list.png" /> Allotment</a>
                </li>
            </ul>
        </nav>
        <div id="content">
                                ...
                                <tr>
                                    <th class="py-3 bg-light">
                                        <select name="room" class="form-control">
                                            <?php
                                            $select_rooms = "SELECT rid, room_no, floor FROM room";
                                    ...
                                    <th class="py-3 bg-light"><input type="number" name="start" class="form-control"
                                            size=4></th>
                                    <th class="py-3 bg-light"><input type="number" name="end" class="form-control"
                                            size=4></th>
                                    <th class="py-3 bg-light"></th>
                                    <th class="py-3 bg-light"><button class="btn btn-info form-control"
                                            name="addallotment">Add</button></th>
                                </tr>
                            </form>
                            <?php
                            $selectclass = "SELECT * FROM batch, class, room WHERE rid=room_id AND class.class_id=batch.class_id GROUP BY rid, class.class_id";
                            $selectclassquery = mysqli_query($conn, $selectclass);
                            if ($selectclassquery) {
                                while ($row = mysqli_fetch_assoc($selectclassquery)) {
                                    echo "<tr>
